...
directory = 'D:\\Data\\speech\\benchmark_datasets\\buckeye\\data_comp_accuracy_data_align'


csv_files = list.files(path=directory, pattern="*utterance_metrics.txt", full.names=TRUE)

data_comp_align_utterance_metrics = csv_files %>% map_dfr(read_csv)

data_comp_align_utterance_metrics$num_speakers <- as.numeric(str_extract(data_comp_align_utterance_metrics$aligner, '^(\\d+)'))
data_comp_align_utterance_metrics$index <- as.numeric(str_extract(data_comp_align_utterance_metrics$aligner, '(\\d+)$'))

data_comp_align_utterance_metrics$phone_boundary_error <- data_comp_align_utterance_metrics$overlap_error /2

data_comp_align_utterance_metrics$corrected_num_insertions <- data_comp_align_utterance_metrics$num_insertions

remove_utterances = c('s4003b_205.2415_223.195437')

data_comp_align_utterance_metrics <- subset(data_comp_align_utterance_metrics, !utterance %in% remove_utterances)


csv_files = list.files(path=directory, pattern="*word_distance.txt", full.names=TRUE)

data_comp_align_word_distances = csv_files %>% map_dfr(read_csv)


data_comp_align_word_distances$num_speakers <- as.numeric(str_extract(data_comp_align_word_distances$aligner, '^(\\d+)'))
data_comp_align_word_distances$index <- as.numeric(str_extract(data_comp_align_word_distances$aligner, '(\\d+)$'))


data_comp_align_word_distances <- subset(data_comp_align_word_distances, !(discourse == 's4003b' & reference_time > 205 & reference_time < 223.2))
data_comp_align_word_distances <- subset(data_comp_align_word_distances, !(discourse == 's2401a' & reference_time > 36.49 & reference_time < 51.2))
data_comp_align_word_distances <- subset(data_comp_align_word_distances, !(discourse == 's3301a' & reference_time > 28.2 & reference_time < 44.1))
data_comp_align_word_distances <- subset(data_comp_align_word_distances, !(discourse == 's2903b' & reference_time > 518))

data_comp_align_word_distances$distance <- abs(data_comp_align_word_distances$distance)


csv_files = list.files(path=directory, pattern="*phone_distance.txt", full.names=TRUE)

data_comp_align_phone_distances = csv_files %>% map_dfr(read_csv)

data_comp_align_phone_distances$distance <- abs(data_comp_align_phone_distances$distance)


data_comp_align_phone_distances$num_speakers <- as.numeric(str_extract(data_comp_align_phone_distances$aligner, '^(\\d+)'))
data_comp_align_phone_distances$index <- as.numeric(str_extract(data_comp_align_phone_distances$aligner, '(\\d+)$'))

data_comp_align_phone_distances$type <- factor(data_comp_align_phone_distances$type, levels=c('initialc', 'cv', 'vc', 'finalc'), labels=c('Initial C', 'CV transition', 'VC transition', 'Final C'))
...
plotData <- summarySE(data=data_comp_align_utterance_metrics, measurevar = 'phone_boundary_error', groupvars=c('num_speakers'))
...
ggplot(aes(x=num_speakers, y=mean * 1000), data=plotData) +
  ...
  ylab('Phone boundary error (ms)') + xlab('Number of training speakers') +ggtitle('Phone boundary errors on held out data') +
...
ggsave('C:\\Users\\michael\\Documents\\Dev\\Web\\memcauliffe.com\\memcauliffe-pelican\\content\\images\\data_comp_align_average_phone_boundary_error.svg',width = blog_width, height = blog_height, units = 'px', dpi =blog_dpi)


plotData <- summarySE(data=data_comp_align_word_distances, measurevar = 'distance', groupvars=c('num_speakers', 'pause_boundary'))
...
ggplot(aes(x=num_speakers, y=mean * 1000, color = pause_boundary, shape = pause_boundary, group=pause_boundary), data=plotData)+
  ...
  ylab('Distance to annotated time point (ms)') + scale_color_manual(name = 'Boundary with silence', values = cbbPalette) + scale_fill_manual(name = 'Boundary with silence', values = cbbPalette) + ggtitle("Word boundary errors on held out data") +
  scale_shape_discrete(name = 'Boundary with silence') + xlab('Number of training speakers') +theme_memcauliffe()
ggsave('C:\\Users\\michael\\Documents\\Dev\\Web\\memcauliffe.com\\memcauliffe-pelican\\content\\images\\data_comp_align_word_boundaries.svg',width = blog_width, height = blog_height, units = 'px', dpi =blog_dpi)

plotData <- summarySE(data=data_comp_align_phone_distances, measurevar = 'distance', groupvars=c('num_speakers','type'))
...
ggplot(aes(x=num_speakers, y=mean * 1000, color = type, shape=type), data=plotData)+
  ...
  theme_memcauliffe() + ggtitle('Phone boundary errors in selected CVC words in held out data')
ggsave('C:\\Users\\michael\\Documents\\Dev\\Web\\memcauliffe.com\\memcauliffe-pelican\\content\\images\\data_comp_align_cvc_phone_boundaries.svg',width = blog_width, height = blog_height, units = 'px', dpi =blog_dpi)


plotData <- summarySE(data=align_benchmark, measurevar = '`Final log-likelihood`', groupvars=c('num_speakers'))
...
ggplot(aes(x=num_speakers, y=mean), data=plotData) +geom_point(size = 5, colour = "#FB5607") + 
  ...
  ggtitle("Final alignment log-likelihood on held out data")
ggsave('C:\\Users\\michael\\Documents\\Dev\\Web\\memcauliffe.com\\memcauliffe-pelican\\content\\images\\data_comp_align_log_like.svg',width = blog_width, height = blog_height, units = 'px', dpi =blog_dpi)


plotData <- summarySE(data=subset(align_benchmark, `Total time` < 50000 & !Corpus %in% c('6_4')), measurevar = '`Total time`', groupvars=c('num_speakers'))
ggplot(aes(x=num_speakers, y=(mean /60)/60), data=plotData) + geom_point(size = 5, colour = "#FB5607") + 
  ...
  ylab('Time (hours)')+ xlab('Number of training speakers') + theme_bw() + ggtitle("Time to generate alignment on held out data") + theme_memcauliffe() +scale_y_log10()
  #+ scale_y_continuous(limits=c(0, 1.6))
ggsave('C:\\Users\\michael\\Documents\\Dev\\Web\\memcauliffe.com\\memcauliffe-pelican\\content\\images\\data_comp_align_timing.svg',width = blog_width, height = blog_height, units = 'px', dpi =blog_dpi)
